...
const envSchema = z.object({
  ...
  EMAIL_HOST: z.string().optional().default(process.env.SMTP_HOST || ''),
  EMAIL_PORT: z.coerce.number().default(Number(process.env.SMTP_PORT) || 587),
  EMAIL_USER: z.string().optional().default(process.env.SMTP_USER || ''),
  EMAIL_PASSWORD: z.string().optional().default(process.env.SMTP_PASS || process.env.SMTP_PASSWORD || ''),
  EMAIL_FROM: z.string().default(process.env.SMTP_FROM || 'Financial Flow <noreply@financialflow.io>'),
